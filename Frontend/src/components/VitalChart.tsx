...
export function VitalChart({ data, title, unit, color, normalRange }: VitalChartProps) {
  ...
  // Prepare min/max lines data if normalRange is provided
  const linesData = normalRange
    ? data.map(d => ({
        ...d,
        minLine: normalRange.min,
        maxLine: normalRange.max,
      }))
    : data;
      ...
      <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2 mt-1">
            <span
              className={`text-2xl font-bold ${
                isCurrentAbnormal ? 'text-destructive animate-pulse-glow' : 'text-primary'
              }`}
            >
              {latestValue ?? '--'}
            </span>
        ...
        <div
          className={`w-3 h-3 rounded-full ${
            isCurrentAbnormal ? 'bg-destructive animate-pulse-glow' : 'bg-success'
          }`}
        />
      </div>
